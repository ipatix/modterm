...
uint8_t getkey_by_per(uint16_t period) {
    if (period == 0) return KEY_NONE;
    for (uint8_t i = 0; i < KEY_RANGE; i++) {
        if (period == period_table[0][i]) {
            return i;
        }
    }
    return KEY_INVALID;
...
const uint16_t period_table[TUNE_RANGE][KEY_RANGE] = {
